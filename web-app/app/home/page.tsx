...
export default function HomePage() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      if (!mercurioId) return;
      try {
        const convs = await fetchConversations(mercurioId);
        setConversations(convs);
      } catch {}
    }, 3000);
  ...
  const getContactName = (otherId: string) => {
    ...
    return contact?.display_name || `${otherId.substring(0, 12)}...`;
  };
  ...
  const handleStartChat = async (contact: Contact) => {
    if (!mercurioId) return;
    const conversationId = generateConversationId(mercurioId, contact.contact_mercurio_id);
    const sorted = [mercurioId, contact.contact_mercurio_id].sort();
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="bg-dark-lighter border-b border-white/10 px-6 py-4 flex-shrink-0">
          ...
          <div>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 px-6 gap-4">
                <div className="w-20 h-20 gradient-bg rounded-full flex items-center justify-center opacity-40">
                ...
                <p className="text-gray-400 text-center text-sm">
                  ...
                  <br />Share your Mercurio ID with others.
                </p>
                <Link
                  href="/add-contact"
                  className="px-6 py-3 gradient-bg rounded-xl text-black font-semibold hover:opacity-90 transition-opacity"
                >
                  + Add Contact
                </Link>
              </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-400 truncate">
                          {conv.last_message ? '🔒 ' + conv.last_message : 'No messages yet — say hello!'}
                        </p>
                      </div>
                    </Link>
                  );
          ...
          <div>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 px-6 gap-4">
                ...
                <p className="text-gray-400 text-sm">Tap + to add a contact</p>
                <Link
                  href="/add-contact"
                  className="px-6 py-3 gradient-bg rounded-xl text-black font-semibold hover:opacity-90 transition-opacity"
                >
              ...
              <div className="divide-y divide-white/10">
                {contacts.map((contact) => (
                  ...
                  >
                    <div className="w-12 h-12 gradient-bg rounded-full flex items-center justify-center flex-shrink-0">
                      <span className="text-black font-bold text-lg">{contact.display_name[0].toUpperCase()}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-white truncate">{contact.display_name}</h3>
                      <p className="text-xs text-gray-500 font-mono truncate">{contact.contact_mercurio_id.substring(0, 20)}...</p>
                    </div>
        ...
        {activeTab === 'settings' && (
          <div className="p-6 space-y-6">
            <div className="flex flex-col items-center py-8">
              ...
              <h2 className="text-xl font-bold text-white mb-1">Mercurio User</h2>
              {mercurioId && (
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-xs text-gray-500 font-mono">{mercurioId.substring(0, 20)}...</p>
                </div>
              )}
            </div>
            ...
            {mercurioId && (
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <p className="text-xs text-gray-400 mb-2 font-medium">Your Mercurio ID</p>
                <p className="text-xs text-gray-300 font-mono break-all leading-relaxed">{mercurioId}</p>
                <button
                  onClick={copyId}
                  className="mt-3 w-full py-2 text-sm text-primary border border-primary/30 rounded-lg hover:bg-primary/10 transition-colors flex items-center justify-center gap-2"
                >
                  {copySuccess ? (
                    <>
      ...
      <nav className="bg-dark-lighter border-t border-white/10 px-6 py-3 flex-shrink-0">
        <div className="flex justify-around">
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg transition-colors ${
              activeTab === 'contacts' ? 'text-primary' : 'text-gray-400 hover:text-white'
            }`}
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
